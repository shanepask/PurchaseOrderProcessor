...
namespace PurchaseOrderProcessor.Infrastructure
{
    public static class ServicesExtension
    {
        ...
        public static IServiceCollection AddPurchaseOrderProcessor(this IServiceCollection services, IOptions<CustomerApiClient.Settings> options)
        {
            ...
            if (options.Value.MockHostForDemo)
            {
                services.AddHttpClient<ICustomerClient, CustomerApiClient>()
                    .ConfigureHttpClient(c => c.BaseAddress = new Uri(options.Value.BaseUrl)).ConfigurePrimaryHttpMessageHandler(() => new DemoHttpHandler());
            }
            else
            {
                services.AddHttpClient<ICustomerClient, CustomerApiClient>()
                    .ConfigureHttpClient(c => c.BaseAddress = new Uri(options.Value.BaseUrl));
            }
